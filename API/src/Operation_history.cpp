...
nlohmann::json Operation_history::req()
{
    ...
    std::vector<std::string> types = {"outgoing-transfer", "deposition", "incoming-transfer"};
    if (std::find(types.begin(), types.end(), this->type) != types.end())
    ...
    if (!this->till.empty())
        payload.Add({"till", till});
    if (this->start_record >= 0)
        payload.Add({"start_record", std::to_string(start_record)});
    if (this->records > 0)
        payload.Add({"records", std::to_string(records)});
...
Operation_history::Operation_history(std::string api_key, std::string method, std::string type, std::string label,
                                     std::string from, std::string till, long start_record, long records)
{
    ...
    if (data.contains("error"))
    {
        if (data["error"] == "illegal_param_type")
            throw IllegalParamType();
        else if (data["error"] == "illegal_param_start_record")
            throw IllegalParamStartRecord();
        else if (data["error"] == "illegal_param_records")
            throw IllegalParamRecords();
        else if (data["error"] == "illegal_param_label")
            throw IllegalParamLabel();
        else if (data["error"] == "illegal_param_from")
            throw IllegalParamFromDate();
        else if (data["error"] == "illegal_param_till")
            throw IllegalParamTillDate();
        else
            throw Technical();
    }
    if (data.contains("next_record"))
